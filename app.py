...
# ══════════════════════════════════════════════════════════════════════════════
# RUTAS V3 — Parser IA (Gemini Flash) · tabla reports_v3
# ══════════════════════════════════════════════════════════════════════════════
import threading
...
def _upsert_v3(cur, parsed: dict, source_id: int, body_preview: str):
    cur.execute("""
        ...
        DO UPDATE SET
            ...
            medicacion = EXCLUDED.medicacion, estado = EXCLUDED.estado,
            desayuno = EXCLUDED.desayuno, almuerzo = EXCLUDED.almuerzo,
            comida = EXCLUDED.comida, merienda = EXCLUDED.merienda,
            cena = EXCLUDED.cena, actividad = EXCLUDED.actividad,
...
def v3_reports():
    ...
    cur.execute(f"SELECT * FROM reports_v3 WHERE {where_sql} ORDER BY date DESC, mediator LIMIT %s OFFSET %s",
...
def v3_reparse_all():
    ...
    def run_reparse():
        ...
        try:
            cur = conn.cursor()
            q = "SELECT id, date, mediator, turn, body_preview FROM reports WHERE body_preview IS NOT NULL AND body_preview != ''"
            if limit > 0: q += f" LIMIT {limit}"
            cur.execute(q)
            ...
            for i, msg in enumerate(messages):
                try:
                    body_text = msg.get("body_preview") or ""
                    parsed = v3_parse_message(body_text, client=client)
                    parsed["_source_date"] = msg["date"].isoformat() if msg.get("date") else None
                    parsed["_source_mediator"] = msg.get("mediator")
                    wcur = conn.cursor()
                    _upsert_v3(wcur, parsed, msg["id"], body_text)
                    conn.commit()
...
def v3_stats():
    ...
    cur.execute("""
        SELECT COUNT(*) as total,
            ...
            ROUND(AVG(pis)::numeric, 1) as avg_pis,
...
def v3_alimentacion():
    ...
    cur.execute(f"""SELECT date, mediator, turn, desayuno, almuerzo, comida, merienda, cena
        FROM reports_v3 WHERE {" AND ".join(where)} ORDER BY date DESC LIMIT 200""", params)
    rows = [dict(r) for r in cur.fetchall()]
    for r in rows:
...
def migrate_v3_endpoint():
    pwd = request.headers.get('X-Admin-Password','') or request.headers.get('X-Password','') or request.args.get('pwd','')
    if pwd != ADMIN_PASSWORD and pwd != API_PASSWORD:
        return jsonify({'error': 'No autorizado'}), 401
    from migrate_v3 import migrate
    try:
        migrate()
        return jsonify({'ok': True, 'message': 'Tabla reports_v3 creada correctamente'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
